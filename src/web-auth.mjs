...
const telegramKey = createPublicKey({
  key: Buffer.from(
    "302a300506032b6570032100e7bf03a2fa4602af4580703d88dda5bb59f32ed8b02a56c187fe7d34caed242d",
    "hex",
  ),
  format: "der",
  type: "spki",
...
export function telegramUser(
  initData,
  botId,
  { now = Date.now(), key = telegramKey } = {},
) {
  if (
    typeof initData !== "string" ||
    initData.length > 16000 ||
    !/^\d{1,20}$/.test(botId)
  )
    throw Error("Invalid Telegram launch");
  const data = new URLSearchParams(initData),
    seen = new Set();
  for (const [name] of data) {
    if (seen.has(name)) throw Error("Duplicate launch field");
    seen.add(name);
  }
  const signature = data.get("signature");
  if (!signature || !/^[A-Za-z0-9_-]{86}={0,2}$/.test(signature))
    throw Error("Invalid Telegram signature");
  const check = [...data]
    .filter(([name]) => !["hash", "signature"].includes(name))
  ...
  if (!Number.isSafeInteger(date) || date > now + 30000 || date < now - 300000)
    throw Error("Expired Telegram launch");
  ...
  if (
    ...
    throw Error("Invalid Telegram user");
  return user.id;
}
export function webOrigin(value) {
  ...
  if (
    ...
    throw Error("Expected HTTPS origin, or local http://127.0.0.1:PORT");
  return url;
}
export class WebAuth {
  ...
  async login(input) {
    ...
    if (
      ...
      if (!owner || owner.telegramUserId !== user)
        throw Error("Only the paired owner can use Voice");
      const launchId = Buffer.from(
        new URLSearchParams(input.initData).get("signature"),
        "base64url",
      ).toString("hex");
      if (this.used.has(launchId))
        throw Error("Launch already used; reopen Voice");
  ...
  async authorize(token) {
    ...
    if (session.owner) {
      ...
      if (
        !owner ||
        owner.telegramUserId !== session.owner.telegramUserId ||
        owner.pairedAt !== session.owner.pairedAt
      ) {
        this.sessions.delete(token);
        throw Error("Owner access revoked");
      }
    }
    return token;
  }
