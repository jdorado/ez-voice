...
export async function serveWeb({
  ...
  const server = http.createServer(async (req, res) => {
    const reply = (status, value, type = "application/json") => {
      res.writeHead(status, {
        ...
        "Content-Security-Policy":
          "default-src 'self'; script-src 'self' https://telegram.org; style-src 'self'; connect-src 'self'; media-src 'self' blob:; frame-ancestors https://web.telegram.org",
      });
    ...
    try {
      ...
      if (req.method !== "POST" || req.headers.origin !== origin)
        return reply(403, { error: "Expected same-origin POST" });
      let raw = "";
      for await (const chunk of req) {
        raw += chunk;
        if (Buffer.byteLength(raw) > 110000)
          return reply(413, { error: "Request too large" });
      }
      const input = JSON.parse(raw || "{}");
      if (url.pathname === "/auth") {
        try {
          return reply(200, await auth.login(input));
        } catch {
          return reply(401, { error: "Access denied; reopen Voice" });
        }
      }
      ...
      reply(400, { error: "Voice request failed" });
    }
  });
  ...
  const lease = setInterval(() => {
    ...
    void (async () => {
      let method = "ping";
      try {
        await auth.authorize(sessionToken);
      } catch {
        method = "stop";
      }
      if (Date.now() - lastBrowserSeen > 15000) method = "stop";
      await request(method);
      if (method === "stop") sessionId = undefined;
    })().catch(() =>
      append({ type: "error", message: "Voice connection unavailable" }),
    );
  }, 5000);
  return {
    server,
    append,
    authorizeActive: () => auth.authorize(sessionToken),
    close: async () => {
      clearInterval(lease);
      if (sessionId || starting) await request("stop").catch(() => {});
